package Servlets;

import Dao.GroupDataStorage;
import Model.Group;
import Model.User;

import javax.servlet.ServletException;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
import javax.servlet.http.HttpSession;
import java.io.IOException;

public class NewGroupServlet extends HttpServlet {
    private static boolean checkvalidate = false;
    private static int checksuccess = 0;

    @Override
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        req.setAttribute("checkvalidate", checkvalidate);
        checkvalidate = false;
        req.setAttribute("checksuccess", checksuccess);
        checksuccess = 0;
        req.getRequestDispatcher("/newgroup.jsp").forward(req, resp);
    }

    @Override
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        HttpSession httpSession = req.getSession();
        User thisUser = (User) httpSession.getAttribute("thisuser");
        Group new_group = new Group();
        String groupName = req.getParameter("groupName");
        String groupDescription = req.getParameter("groupDescription");
        new_group.setGroupName(groupName);
        new_group.setGroupDescription(groupDescription);
        new_group.setGroupLeader(thisUser.getuserId());
        GroupDataStorage.addGroup(new_group);
        resp.sendRedirect("/groupselection");
    }
}
